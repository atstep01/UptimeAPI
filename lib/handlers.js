...
handlers.__users.get = function(data,callback){
	...
	if(phone){
		...
		handlers.__tokens.verifyToken(token, phone, function(tokenIsValid){
			if(tokenIsValid){
				...
				callback(403, {'Error' : 'Token is not in header or is invalid'});
			}
		});
...
handlers.__tokens.verifyToken = function(Id, phone, callback){
	//Lookup token.
	__data.read('tokens', Id, function(err, tokenData){
		if(!err && tokenData){

			//Check that a token is for a given user and hasn't expired
